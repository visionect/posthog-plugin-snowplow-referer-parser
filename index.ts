...
function processEvent(event) {
  ...
  if (
    ...
    console.log(`(DIRECT) Source: direct`)
  ...
  const searchQuery = getSearchQueryFromUrl(
    props.$referrer,
    referrerData.queryParams,
  )

  console.log(
    `(SNOWPLOW) Medium: ${referrerData.medium}, Source: ${referrerData.source}, Term: ${searchQuery}`,
  )
...
function getMediumAndSourceFromReferrer(domain) {
  let medium = "unknown"
  let source = "unknown"
  let queryParams = []

  for (let med of Object.keys(referrers)) {
    for (let src of Object.keys(referrers[med])) {
...
function getSearchQueryFromUrl(url, queryParams) {
  let searchQuery = ""
  ...
  for (let param of queryParams) {
    let searchString = getParameterByName(param, url)
...
function isReferrerFromDirectDomain(referrer) {
  let isDirect = false

  for (let domain of directReferrerDomains) {
...
function test(options = { verbose: false }) {
  ...
  console.log("--- UTM test")

  const processedUTMEvent = processEvent(utmEvent)

  console.log(
    "TEST: Processed UTM Event",
    processedUTMEvent.properties.referrer_parser === "utm",
  )
  isVerbose && console.log(JSON.stringify(processedUTMEvent))

  console.log("--- Snowplow test")
  ...
  console.log(
    "TEST: Processed Referrer Event",
    processedReferrerEvent.properties.referrer_parser === "snowplow",
  )
  isVerbose && console.log(JSON.stringify(processedReferrerEvent))
  ...
  console.log(
    "TEST: Processed Direct Event",
    processedDirectEvent.properties.referrer_parser ===
      "direct_and_own_domains",
  )
  isVerbose && console.log(JSON.stringify(processedDirectEvent))
}
